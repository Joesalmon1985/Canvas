...
class CouncilUpdate:
    ...
    def addingpostcodecol (self, databaseused):
    ...
    def addingpostcodedata (self, databaseused):
        with sqlite3.connect( databaseused ) as conn:
           cursor = conn.cursor()
           tobedone = '''select ROWID,* from councilupdated'''
           cursor.execute( tobedone )
           # Does not work
           #c = cursor.rowcount
           #print "%r rows" % ( c )
           for row in cursor:
               ...
               for column in row:
                   ...
                   a = str( column )
#                   print "218 %r" % ( a )
                   if re.match('[A-Z]{1,2}[0-9]{1,2} [0-9]{1,2}[A-Z]{2}', a):
                       print "FOUND A POSTCODE [%r]" % ( a )
                       tobedone = "UPDATE councilupdated SET postcode='%s' WHERE ROWID=%r " % ( a, row[ 0 ] )
                       print tobedone
                       cursorUpdate.execute( tobedone )
#                       break
               print "Next row"
